...
const checkoutFormSchema = z.object({
  ...
  communes: z.string({ required_error: 'Vui lòng chọn Xã/Phường.' }),
  address: z.string().min(5, 'Vui lòng nhập địa chỉ chi tiết.'),
  paymentMethod: z.enum(['COD', 'TRANSFER', 'PAYMENT_AT_STORE']),
...
export default function CheckoutPage() {
  ...
  const selectedCityCode = form.watch('province');
  ...
  async function onSubmit(values: CheckoutFormValues) {
    ...
    const fullAddress = {
      ...values,
      province: cities.find(c => c.value === values.province)?.label,
      district: districts.find(d => d.value === values.district)?.label,
      communes: wards.find(w => w.value === values.communes)?.label,
    }

    const listOrderProduct = cartItems.map(item => ({ productCode: item.productCode, quantity: item.quantity, productName: item.productName, price: item.price }))
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                ...
                <ControlledInput control={form.control} name="fullName" label="Họ và tên *" />
                <div className="grid grid-cols-2 gap-4">
                  <ControlledInput control={form.control} name="phoneNumber" label="Số điện thoại *" />
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <ControlledRadioGroup
                    ...
                    label=""
                    options={[
                      { value: 'TRANSFER', label: 'Chuyển khoản qua máy ATM & Ngân hàng' },
                      { value: 'PAYMENT_AT_STORE', label: 'Nhận hàng và thanh toán tại maythucpham365' },
                      { value: 'COD', label: 'Giao hàng và thu tiền tại nhà' },
                    ]}
                  />
                </div>
                <div>
                ...
                {cartItems.map(item => (
                  <div key={item.id} className="flex items-center gap-4 text-sm">
                    ...
                    <div className="flex-grow">
                      ...
                      <p className="text-xs text-muted-foreground">{item.typeName || 'Standard'}</p>
                    </div>
